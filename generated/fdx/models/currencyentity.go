// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"context"

	"github.com/go-openapi/errors"
	"github.com/go-openapi/strfmt"
	"github.com/go-openapi/swag"
)

// Currencyentity Currencyentity
//
// Represents an international currency
//
// swagger:model Currencyentity
type Currencyentity struct {

	// currency code
	CurrencyCode ISO4217Code1 `json:"currencyCode,omitempty"`

	// Currency rate between original and converted currency
	CurrencyRate float64 `json:"currencyRate,omitempty"`

	// original currency code
	OriginalCurrencyCode ISO4217Code2 `json:"originalCurrencyCode,omitempty"`
}

// Validate validates this currencyentity
func (m *Currencyentity) Validate(formats strfmt.Registry) error {
	var res []error

	if err := m.validateCurrencyCode(formats); err != nil {
		res = append(res, err)
	}

	if err := m.validateOriginalCurrencyCode(formats); err != nil {
		res = append(res, err)
	}

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}

func (m *Currencyentity) validateCurrencyCode(formats strfmt.Registry) error {
	if swag.IsZero(m.CurrencyCode) { // not required
		return nil
	}

	if err := m.CurrencyCode.Validate(formats); err != nil {
		if ve, ok := err.(*errors.Validation); ok {
			return ve.ValidateName("currencyCode")
		} else if ce, ok := err.(*errors.CompositeError); ok {
			return ce.ValidateName("currencyCode")
		}
		return err
	}

	return nil
}

func (m *Currencyentity) validateOriginalCurrencyCode(formats strfmt.Registry) error {
	if swag.IsZero(m.OriginalCurrencyCode) { // not required
		return nil
	}

	if err := m.OriginalCurrencyCode.Validate(formats); err != nil {
		if ve, ok := err.(*errors.Validation); ok {
			return ve.ValidateName("originalCurrencyCode")
		} else if ce, ok := err.(*errors.CompositeError); ok {
			return ce.ValidateName("originalCurrencyCode")
		}
		return err
	}

	return nil
}

// ContextValidate validate this currencyentity based on the context it is used
func (m *Currencyentity) ContextValidate(ctx context.Context, formats strfmt.Registry) error {
	var res []error

	if err := m.contextValidateCurrencyCode(ctx, formats); err != nil {
		res = append(res, err)
	}

	if err := m.contextValidateOriginalCurrencyCode(ctx, formats); err != nil {
		res = append(res, err)
	}

	if len(res) > 0 {
		return errors.CompositeValidationError(res...)
	}
	return nil
}

func (m *Currencyentity) contextValidateCurrencyCode(ctx context.Context, formats strfmt.Registry) error {

	if err := m.CurrencyCode.ContextValidate(ctx, formats); err != nil {
		if ve, ok := err.(*errors.Validation); ok {
			return ve.ValidateName("currencyCode")
		} else if ce, ok := err.(*errors.CompositeError); ok {
			return ce.ValidateName("currencyCode")
		}
		return err
	}

	return nil
}

func (m *Currencyentity) contextValidateOriginalCurrencyCode(ctx context.Context, formats strfmt.Registry) error {

	if err := m.OriginalCurrencyCode.ContextValidate(ctx, formats); err != nil {
		if ve, ok := err.(*errors.Validation); ok {
			return ve.ValidateName("originalCurrencyCode")
		} else if ce, ok := err.(*errors.CompositeError); ok {
			return ce.ValidateName("originalCurrencyCode")
		}
		return err
	}

	return nil
}

// MarshalBinary interface implementation
func (m *Currencyentity) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *Currencyentity) UnmarshalBinary(b []byte) error {
	var res Currencyentity
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}
